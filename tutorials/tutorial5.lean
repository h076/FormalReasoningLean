...
begin
  assume m n,
  assume eq,
  injection eq, -- as succ is injective, succ m = succ n → m = n
end   
...
begin
  assume m n,
  assume eq,
  -- rewrite eq would also end the proof here
  apply congr_arg, -- can be applied as our goal is succ m = succ n and we have m = n
  -- can also write apply congr_arg succ, to be specific
  assumption,
end
...
begin
  ...
  -- we want to flip the positions of b and c
  -- changing the left to resemble the right
  -- using associativitly to change bracketing
  rewrite add_assoc a b c, -- apply to an l m n (a b c)
...
begin
  assume l m n,
  induction n with n' ih, -- we use n as it is concrete on both sides and is on the right of addition
  dsimp [add],
  assume eq,
  exact eq,
  -- if you can prove without ih the induction is not needed
  dsimp [add],
  assume eq,
  apply ih,
  injection eq,
...
begin
  ...
  apply congr_arg succ,
  rewrite ih,
  -- now use lemma to prove if stuck, proving simmilar 
  -- we can just use add_succ_lem
